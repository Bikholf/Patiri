<script lang="ts">
  ...
  let { groupData, groupForm, isOpen = $bindable() } = $props();
    ...
    const { form: formData, enhance, form, delayed, submitting } = leaveGroupForm;
...
<Dialog.Root bind:open={isOpen}>
  ...
  <Dialog.Content class="sm:max-w-[425px]">
    ...
    >
        ...
        <Dialog.Footer>
            <Button type="button" variant="outline" class="cursor-pointer" onclick={closeDialog}>{m.cancel()}</Button>
            <Button type="submit" variant="destructive" class="cursor-pointer">{m.leave_group()}</Button>
        </Dialog.Footer>
    </form>
  </Dialog.Content>
</Dialog.Root>